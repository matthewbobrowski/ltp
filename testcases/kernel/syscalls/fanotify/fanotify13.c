 ...
 *	is to validate whether any additional forbidden flags passed with
 *	FAN_UNPRIVILEGED return the correct results.
 */
#define _GNU_SOURCE
#include "config.h"
...
static void do_test(unsigned int n)
{
	...
	if (fd_notify < 0) {
		if (errno == EPERM && tc->init_flags & FAN_UNPRIVILEGED) {
			...
			switch (tc->init_flags & DISALLOWED_INIT_FLAGS) {
			case FAN_UNLIMITED_QUEUE:
			case FAN_UNLIMITED_MARKS:
	...
	if (fanotify_mark(fd_notify, tc->mark_flags, tc->mark_mask, AT_FDCWD,
				filename) < 0) {
		/* Unprivileged users are only allowed to mark inodes */
		if (errno == EINVAL && tc->mark_flags & DISALLOWED_MARK_FLAGS) {
	...
	tst_res(TPASS,
		"fanotify_init() and fanotify_mark() return successfully, "
		"as expected");
}
