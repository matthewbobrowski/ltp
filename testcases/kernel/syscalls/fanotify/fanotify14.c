...
static struct test_case_t {
	...
	{
		"init flags: FAN_UNPRIVILEGED, mask: FAN_ALL_EVENTS (in child)",
		1,
		4,
		{
			FAN_OPEN,
...
static void do_test(unsigned int n)
{
	...
	/* Initialize fanotify */
	fd_notify = fanotify_init(FAN_CLASS_NOTIF | FAN_UNPRIVILEGED, O_RDONLY);
	...
	if (fd_notify < 0) {
		if (errno == EPERM) {
			...
			tst_brk(TBROK | TERRNO,
				...
				"FAN_UNPRIVILEGED, O_RDONLY) failed");
		}
	}
